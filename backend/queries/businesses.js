...
    try {
        let busniessId = req.params.id;
        let business = await db.one(`SELECT * FROM businesses WHERE id=${businessId}`);
        res.status(200).json({
            status: "success",
...
module.exports = { getAllBusiness, getSingleBusiness , createBusiness, editBusiness, deleteBusiness };