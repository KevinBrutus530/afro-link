...
const getAllReviews = async (req, res, next) => {
    ...
        try {
            let business = await db.any(`SELECT * FROM reviews WHERE id=${reviewId}`);
            res.status(200).json({
                status: "success",
    ...
        try {
            let {reviewId} = req.params.id;
            let review = await db.none("DELETE FROM businesses WHERE id=$1 RETURNING *", reviewId);
            res.status(200).json({
                status: "success",
    ...
            let review = await db.one("UPDATE reviews SET review_id=$1, text=$2 name=$3, ratings=$4, zip=$5 WHERE id=$3", [review_id, text, name, ratings, zip, reviewId]);
            res.status(200).json({
                status: "success",
